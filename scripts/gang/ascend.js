...
export async function main(ns) {
  ...
  if (done.length) {
    report(ns, "ascend", `${done.length} ascended (${done.join(", ")}) | respect now ${ns.format.number(respect, 2, 1000, true)}`);
  } else if (blocked) {
    report(ns, "ascend",
      `${blocked} ready at x${best.toFixed(2)} but HELD - ascending would drop respect ` +
        `under the ${ns.format.number(state.nextRecruitAt, 2, 1000, true)} needed for the next recruit`);
  } else {
